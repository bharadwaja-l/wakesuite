...
(function(){
  ...
  function dashboardCategoryMetrics(snaps,market){
    const parity={},disp={},supp={},buy={},inv={},total={};
    for(const s of snaps||[]){
      // Compute price observations ONCE per snapshot/marketplace. The previous
      // implementation recalculated the full observation set once for every
      // SKU row, which became O(n²) and could hang/crash Chrome on production data.
      const observations=(v4PriceObservations(s,market,'all')||[]).filter(o=>allowedCategory(o.category));
      for(const o of observations){
        const cat=o.category||'Unmapped';
        parity[cat]??={p:0,d:0};
        if(o.parityStatus==='Parity'){
          parity[cat].p++;
        }else if(o.parityStatus==='Disparity'){
          parity[cat].d++;
          disp[cat]=(disp[cat]||0)+1;
          total[cat]=(total[cat]||0)+num(o.priceImpact);
        }
      }
      if(market==='amazon'){
        for(const r of getSnapshotAmazonIssueRows(s,'amazonSuppressions')||[]){
          if(!allowedCategory(r.category))continue;
          const cat=r.category||'Unmapped';
          supp[cat]=(supp[cat]||0)+1;
          total[cat]=(total[cat]||0)+num(r.revenueImpactPerDay);
        }
        for(const r of getSnapshotAmazonIssueRows(s,'amazonBuyBox')||[]){
          if(!allowedCategory(r.category))continue;
          const cat=r.category||'Unmapped';
          buy[cat]=(buy[cat]||0)+1;
          total[cat]=(total[cat]||0)+num(r.revenueImpactPerDay);
    ...
    const latest=[...(snaps||[])].sort((a,b)=>String(a.reportDate).localeCompare(String(b.reportDate))).at(-1);
    if(latest){
      for(const r of v7SnapshotInventoryRows(latest,market)||[]){
        if(!allowedCategory(r.category))continue;
        const stock=r.inventoryKnown===false?null:Number(r.inventory);
        if(stock!=null&&stock<=7){
          const cat=r.category||'Unmapped';
          inv[cat]=(inv[cat]||0)+1;
        }
      }
    ...
    const parityPct={};
    for(const [c,x] of Object.entries(parity))parityPct[c]=(x.p+x.d)?(100*x.p/(x.p+x.d)):0;
    return {parityPct,disp,supp,buy,inv,total};
  }
  try{
