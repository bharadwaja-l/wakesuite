...
- No Shared Analytics Filter Bar. Existing page-specific filter/dropdown layouts are retained.
- Added dedicated analytical destinations for Dashboard Price Disparity, Amazon Suppression, Amazon Buy Box and Revenue Impact cards.
- Fixed Dashboard Price Change Performance routing so it opens Pricing Insights.
- Rebuilt Pricing Insights around marketplace Order Reports for orders/units and Amazon order revenue, with 7/14/30 before/after windows and interference-aware decision signals.
- Rebuilt Inventory Insights with Order Report Units Sold/DRR plus inventory-snapshot stock position, 7/14/30 DRR, OOS %, Days of Cover and fulfilment.
- Added exact-date, exception-aware Live Price Disparity order impact to the dedicated Price Disparity analysis page.
- Corrected Data Center source labels to canonical Business Reports / Order Reports naming without changing the Data Center dropdown UI.
- Flipkart monetary revenue/impact remains unavailable when no approved monetary source exists; WakeSuite does not estimate it from quantity × listing price.
...
- `css/` — base/responsive/V9.3/V9.3.2/V9.3.3/V9.3.4 UI styles
- `js/wakesuite-app.js` — proven shared processing/runtime layer
- `js/wakesuite-v9.3.4.js` — V9.3.4 dedicated analytics and Business Insights layer
