...
WakeSuite is the internal marketplace operations and analytics application for Amazon and Flipkart workflows. V9.3.5 is the current consolidated operations and analytics build. AI / Ask WakeSuite is intentionally excluded.
...
## V9.3.5 reporting, disparity and inventory update

- Adds Weekly / Monthly Marketplace Reports: internal Wakefit receives Amazon + Flipkart; Amazon and Flipkart POCs receive marketplace-only data.
- Adds exact-date orders and units during actionable Live Price Disparity to periodic reporting.
- Flipkart uses the approved Audit / Listing Live Price as the basis for `Order Value at Live Price`; it is not labelled actual order revenue.
- Active pricing exceptions are excluded before disparity impact/order-value aggregation.
- Adds Amazon FBA Holding Risk with 7/14/30-day FBA sales, last FBA sale, days since sale, 30-day DRR, days cover, excess units, Slow Moving / Non-Moving states and related-issue context.
- FBA storage-fee rupee estimates remain excluded until an approved Amazon ageing/fee source is connected.
- Adds Super Admin Drive endpoint/folder configuration hooks for centrally owned POA/QC files.
- Buy Box POC WF Price is resolved from canonical Master Pricing where available; revenue-impact outputs use whole-number values.

## V9.3.4 Daily Communications update

- Amazon POC escalation is consolidated into one daily email instead of separate Live Price Disparity, ASIN Suppression and Buy Box emails.
- The email body contains only one summary table: Issue, No. of Issues and Revenue Impact / Day.
- Salutation uses the Amazon POC Name configured under Settings → System → Operational Controls, with `Team` as the fallback.
- Live Price Disparity is attached separately in the operational structure `Category | Seller sku | ASIN | Price`; revenue impact is intentionally not added to that attachment.
- ASIN Suppression is attached separately as `Category | ASIN | Rev Impact | POA Link | Escalated On`. Suppression Management now supports an optional POA Link field.
- Buy Box Suppression is attached separately as `Category | ASIN | WF Item SKU | WF Price | Rev Impact`.
- An attachment is created only when that issue type has at least one issue. Zero-issue types remain visible as 0 / ₹0 in the summary.
- The closing action line is: `Please find the attached files and take necessary actions and let us know once done.`
- Existing Amazon/Flipkart follow-up and internal Daily Marketplace Report workflows are preserved.
