...
const V7_ACTION_KEYS = [
  ...
  "manageSuppressions","managePricingExceptions","settings","userAdmin"
];
const V7_ROLE_PRESETS = {
  viewer:{view:true,upload:false,download:true,email:false,suppressionOverride:false,pocEscalation:false,manageSuppressions:false,managePricingExceptions:false,settings:false,userAdmin:false},
  analyst:{view:true,upload:true,download:true,email:true,suppressionOverride:false,pocEscalation:false,manageSuppressions:false,managePricingExceptions:false,settings:false,userAdmin:false},
  admin:{view:true,upload:true,download:true,email:true,suppressionOverride:true,pocEscalation:true,manageSuppressions:true,managePricingExceptions:true,settings:true,userAdmin:false},
  super_admin:{view:true,upload:true,download:true,email:true,suppressionOverride:true,pocEscalation:true,manageSuppressions:true,managePricingExceptions:true,settings:true,userAdmin:true}
};
...
function v7ActionCheckboxes(permissions,prefix){
  return V7_ACTION_KEYS.map(key=>`<label><input type="checkbox" data-v7-action="${key}" data-prefix="${escapeHtml(prefix)}" ${permissions?.[key]?"checked":""}> ${escapeHtml({userAdmin:"User Admin",suppressionOverride:"Suppression Override",pocEscalation:"POC Escalation",manageSuppressions:"Manage Suppressions",managePricingExceptions:"Manage Pricing Exceptions"}[key]||key.charAt(0).toUpperCase()+key.slice(1))}</label>`).join("");
}
...
function v7AccessEditor(record){
  ...
  const perms={...(V7_ROLE_PRESETS[role]||V7_ROLE_PRESETS.viewer),...(record.permissions||{})};
  const scopes={...V7_DEFAULT_SCOPES,...(record.scopes||{})};
  ...
  return `<details><summary>Access Scope</summary><div class="v7-access-editor" data-v7-access-editor="${escapeHtml(prefix)}">
    <fieldset><legend>Marketplace & Categories</legend><div class="v7-check-grid"><label><input type="checkbox" data-v7-market="amazon" ${scopes.marketplaces.includes("amazon")?"checked":""}> Amazon</label><label><input type="checkbox" data-v7-market="flipkart" ${scopes.marketplaces.includes("flipkart")?"checked":""}> Flipkart</label></div><div class="v72-category-grid">${categoryChecks}</div></fieldset>
    <fieldset><legend>Actions</legend><div class="v7-check-grid">${v7ActionCheckboxes(perms,prefix)}</div></fieldset>
    <fieldset style="grid-column:1/-1"><legend>Menus / Modules</legend><div class="v7-check-grid">${v7ModuleCheckboxes(scopes,prefix)}</div></fieldset>
  </div></details>`;
}
...
====================================================== */
const V9_AMAZON_TEMPLATE_URL = "./assets/Amazon_Pricing_Template.xlsm";
...
expandAmazonRow=function(a){const r=v8BaseExpandAmazonRow(a);r.minAllowedPrice=a.length>28?a[28]:null;r.maxAllowedPrice=a.length>29?a[29]:null;return r;};
...
async function v9AmazonTemplateWorkbook(){
  if(!v9AmazonTemplateBuffer){
    ...
    if(!response.ok) throw new Error("Amazon pricing template could not be loaded from assets/Amazon_Pricing_Template.xlsm.");
...
async function v8BuildAmazonWorkbook(rows,minPct,maxPct,bulkMode=false){
  ...
  const maxIncrease=Math.max(0,Number(maxPct||5))/100;

  for(let i=0;i<source.length;i++){
    const r=source[i];
    const row=start+i;
    const listing=Number(r.listingPrice||0);
    const wf=Number(r.wfPrice||0);
    const wfMrp=Number(r.wfMrp||r.azMrp||0);
    const mrpCap=Number(r.azMrp||wfMrp||0);
    const min=Math.round((listing*(1-minReduction))*100)/100;
    const maxCandidate=Math.round((listing*(1+maxIncrease))*100)/100;
    const max=mrpCap>0?Math.min(mrpCap,maxCandidate):maxCandidate;

    v8SetCell(sheet,`B${row}`,r.azSku);
    v8SetCell(sheet,`C${row}`,"PartialUpdate");
    v8SetCell(sheet,`E${row}`,r.asin);
    v8SetCell(sheet,`F${row}`,"ASIN");

    // Final Amazon selling/listing price always comes from Wakefit Daily Pricing.
    if(!bulkMode&&r.listingPriceDisparity) v8SetCell(sheet,`W${row}`,wf);
    if(!bulkMode&&r.mrpDisparity){v8SetCell(sheet,`XW${row}`,wfMrp);v8SetCell(sheet,`YI${row}`,wfMrp);}
    if(bulkMode||r.minIssue) v8SetCell(sheet,`MK${row}`,min);
    if(bulkMode||r.maxIssue) v8SetCell(sheet,`MF${row}`,max);
  }

  if(source.length){
    const end=start+source.length-1;
    sheet['!ref']=XLSX.utils.encode_range({s:{r:0,c:0},e:{r:end-1,c:XLSX.utils.decode_col('YI')}});
  }
  return wb;
}
async function downloadAmazonPricingCorrection(){
  ...
  const maxPct=Number(document.getElementById("amazonBulkMaxPct")?.value||5);
  const wb=await v8BuildAmazonWorkbook(rows,minPct,maxPct,false);
...
async function downloadAmazonBulkMinMax(){
  ...
  const wb=await v8BuildAmazonWorkbook(rows,minPct,maxPct,true);
