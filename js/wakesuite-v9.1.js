/* ======================================================
   WakeSuite V9.1.1 · Performance + UI Hotfix
   21 Aug 2026
...
let ws911ExceptionIndexToken="";
let ws911ExceptionIndex=new Map();
const ws911ExceptionRowsCache=new WeakMap();
function ws911ExceptionToken(){
  const rows=typeof v7PricingExceptions!=="undefined"?v7PricingExceptions:[];
  return `${typeof v7PricingExceptionsVersion!=="undefined"?v7PricingExceptionsVersion:"0"}:${rows.length}`;
}
function ws911IndexPush(key,ex){
  if(!key)return;
  if(!ws911ExceptionIndex.has(key))ws911ExceptionIndex.set(key,[]);
  ws911ExceptionIndex.get(key).push(ex);
}
function ws911BuildExceptionIndex(){
  const token=ws911ExceptionToken();
  if(token===ws911ExceptionIndexToken)return;
  ws911ExceptionIndexToken=token;
  ws911ExceptionIndex=new Map();
  const rows=typeof v7PricingExceptions!=="undefined"?v7PricingExceptions:[];
  rows.forEach(ex=>{
    const market=String(ex.marketplace||"all").toLowerCase();
    const prefix=market+":";
    if(ex.wfSku)ws911IndexPush(prefix+"wf:"+ws91Norm(ex.wfSku).toLowerCase(),ex);
    if(ex.asin)ws911IndexPush(prefix+"asin:"+ws91Norm(ex.asin).toLowerCase(),ex);
    if(ex.azSku||ex.marketplaceSku)ws911IndexPush(prefix+"az:"+ws91Norm(ex.azSku||ex.marketplaceSku).toLowerCase(),ex);
    if(ex.fsn)ws911IndexPush(prefix+"fsn:"+ws91Norm(ex.fsn).toLowerCase(),ex);
    if(ex.fkSku||ex.marketplaceSku)ws911IndexPush(prefix+"fk:"+ws91Norm(ex.fkSku||ex.marketplaceSku).toLowerCase(),ex);
    if(ex.identifier)ws911IndexPush(prefix+"id:"+ws91Norm(ex.identifier).toLowerCase(),ex);
    const category=ws91Norm(ex.category).toLowerCase();
    const hasProduct=!!(ex.wfSku||ex.asin||ex.azSku||ex.fsn||ex.fkSku||ex.marketplaceSku||ex.identifier);
    if(category&&!hasProduct&&!['all','*','unmapped'].includes(category))ws911IndexPush(prefix+"cat:"+category,ex);
  });
}
function ws911ExceptionCandidates(row,marketplace){
  ws911BuildExceptionIndex();
  const markets=[marketplace,"all"],keys=[];
  const push=(kind,value)=>{const v=ws91Norm(value).toLowerCase();if(v)markets.forEach(m=>keys.push(`${m}:${kind}:${v}`));};
  push("wf",row.wfSku);
  if(marketplace==="amazon"){push("asin",row.asin);push("az",row.azSku);push("id",row.asin);push("id",row.azSku);}
  else{push("fsn",row.fsn);push("fk",row.fkSku);push("id",row.fsn);push("id",row.fkSku);}
  push("cat",row.category);
  const out=[],seen=new Set();
  keys.forEach(k=>(ws911ExceptionIndex.get(k)||[]).forEach(ex=>{if(!seen.has(ex)){seen.add(ex);out.push(ex);}}));
  return out;
}
function ws91FindException(row,marketplace,target,reportDate){
  return ws911ExceptionCandidates(row,marketplace).find(ex=>{
...
getSnapshotAmazonRows = function(snapshot){
  if(!snapshot)return [];
  const token=ws911ExceptionToken(),cached=ws911ExceptionRowsCache.get(snapshot)||{};
  if(cached.token===token&&cached.amazon)return cached.amazon;
  const date=snapshot?.reportDate||snapshot?.date||todayIso();
  const rows=(ws91BaseGetAmazonRows(snapshot)||[]).map(r=>ws91ApplyRowExceptions({...r},"amazon",date));
  ws911ExceptionRowsCache.set(snapshot,{...cached,token,amazon:rows});
  return rows;
};
...
getSnapshotFlipkartRows = function(snapshot){
  if(!snapshot)return [];
  const token=ws911ExceptionToken(),cached=ws911ExceptionRowsCache.get(snapshot)||{};
  if(cached.token===token&&cached.flipkart)return cached.flipkart;
  const date=snapshot?.reportDate||snapshot?.date||todayIso();
  const rows=(ws91BaseGetFlipkartRows(snapshot)||[]).map(r=>ws91ApplyRowExceptions({...r},"flipkart",date));
  ws911ExceptionRowsCache.set(snapshot,{...cached,token,flipkart:rows});
  return rows;
};
...
const WS911_ACTION_CACHE_MS=120000;
let ws911SuppressionCasesCache={at:0,rows:[]};
let ws911OverridesCache=new Map();

function ws911ApplyOverrideVisuals(snap,date,overrides){
  window.ws91SuppressionOverrides=overrides||[];
  ...
  const supRows=(typeof getSnapshotAmazonIssueRows==="function"?getSnapshotAmazonIssueRows(snap,"amazonSuppressions"):[])
    .filter(r=>!(overrides||[]).some(o=>o.asin===r.asin));
  ws91SetHtml("amazonSuppressionCategoryBreakdown",ws91BreakdownHtml(ws91CategoryCounts(supRows,()=>true)));
}

async function ws911LoadOverrides(date,force=false){
  const cached=ws911OverridesCache.get(date);
  if(!force && cached && Date.now()-cached.at<WS911_ACTION_CACHE_MS)return cached.rows;
  if(typeof window.loadSuppressionOverrides!=="function")return cached?.rows||[];
  const rows=await window.loadSuppressionOverrides(date,date).catch(()=>cached?.rows||[]);
  ws911OverridesCache.set(date,{at:Date.now(),rows:rows||[]});
  return rows||[];
}

async function ws91EnhanceDashboard(){
  const date=document.getElementById("dashboardToDate")?.value||todayIso();
  const snap=await loadSnapshotCached(date).catch(()=>null);
  if(!snap)return;
  ...
  const bb=typeof getSnapshotAmazonIssueRows==="function"?getSnapshotAmazonIssueRows(snap,"amazonBuyBox"):[];
  ws91SetHtml("amazonBuyBoxCategoryBreakdown",ws91BreakdownHtml(ws91CategoryCounts(bb,()=>true)));

  // Fast first paint: use cached workflow data immediately and refresh it in the background.
  const cachedOverrides=ws911OverridesCache.get(date)?.rows||[];
  ws911ApplyOverrideVisuals(snap,date,cachedOverrides);
  ws91RenderActionCenter(snap,az,fk,cachedOverrides,false).catch(()=>{});
  ws911LoadOverrides(date,false).then(overrides=>{
    ws911ApplyOverrideVisuals(snap,date,overrides);
    ws91RenderActionCenter(snap,az,fk,overrides,false).catch(()=>{});
  }).catch(()=>{});
}

function ws911ActionRows(az,fk,cases){
  return [
    {label:"High-impact price disparities",detail:"Prioritize high-revenue pricing issues",count:[...az,...fk].filter(r=>(r.listingPriceDisparity||r.livePriceDisparity||r.mrpDisparity)&&ws91Num(r.dailyRevenueImpact)>=1000).length,go:"openDashboardInsight('combined','price_disparity')"},
    {label:"Suppressions awaiting Case ID",detail:"Cases requiring marketplace follow-up",count:cases.filter(r=>!r.caseId&&!['Reactivated','Closed'].includes(r.status)).length,go:"openSuppressionManagement()"},
    {label:"POC escalations pending",detail:"Escalations requiring action",count:cases.filter(r=>r.pocEscalationStatus==='Required').length,go:"openSuppressionManagement()"},
    {label:"Exceptions expiring soon",detail:"Approved exceptions ending within 7 days",count:(typeof v7PricingExceptions!=="undefined"?v7PricingExceptions:[]).filter(ex=>{if(!ex.effectiveTo)return false;const d=(new Date(ex.effectiveTo)-new Date(todayIso()))/86400000;return d>=0&&d<=7;}).length,go:"openDashboardInsight('combined','exceptions')"}
  ].filter(x=>x.count>0);
}

function ws911PaintActionCenter(actions){
  const box=document.getElementById("actionCenterList");
  if(!box)return;
  box.innerHTML=actions.length?actions.map(a=>`<button class="action-center-item" type="button" onclick="${a.go}"><span class="action-center-count">${a.count}</span><span class="action-center-copy"><strong>${escapeHtml(a.label)}</strong><span>${escapeHtml(a.detail)}</span></span><span class="action-center-arrow">›</span></button>`).join(""):'<div class="utility-empty">No priority actions in the selected data.</div>';
}

async function ws91RenderActionCenter(snap,az,fk,overrides,force=false){
  const now=Date.now();
  ws911PaintActionCenter(ws911ActionRows(az,fk,ws911SuppressionCasesCache.rows||[]));
  if(!force && now-ws911SuppressionCasesCache.at<WS911_ACTION_CACHE_MS)return;
  if(typeof window.loadSuppressionCases!=="function")return;
  try{
    const rows=await window.loadSuppressionCases();
    ws911SuppressionCasesCache={at:Date.now(),rows:rows||[]};
    ws911PaintActionCenter(ws911ActionRows(az,fk,ws911SuppressionCasesCache.rows));
  }catch(_e){}
}

const ws91BaseDashboardOverview=loadDashboardOverview;
loadDashboardOverview=async function(...args){
  const result=await ws91BaseDashboardOverview(...args);
  Promise.resolve().then(()=>ws91EnhanceDashboard()).catch(error=>console.warn("V9.1.1 dashboard enhancement failed",error));
  return result;
};
...
loadDashboardOverview=async function(...args){
  if(typeof v7PricingExceptionsLoaded!=="undefined" && !v7PricingExceptionsLoaded){
    await v7EnsurePricingExceptionsLoaded(false).catch(()=>{});
  }
  return ws91DashboardWithEnhancements(...args);
};
const ws91InsightsWithEnhancements=loadMarketplaceInsights;
loadMarketplaceInsights=async function(...args){
  if(typeof v7PricingExceptionsLoaded!=="undefined" && !v7PricingExceptionsLoaded){
    await v7EnsurePricingExceptionsLoaded(false).catch(()=>{});
  }
  return ws91InsightsWithEnhancements(...args);
};
...
async function refreshActionCenter(){
  const date=document.getElementById("dashboardToDate")?.value||todayIso();
  const snap=await loadSnapshotCached(date).catch(()=>null);if(!snap)return;
  const az=getSnapshotAmazonRows(snap),fk=getSnapshotFlipkartRows(snap);
  const overrides=await ws911LoadOverrides(date,true).catch(()=>[]);
  return ws91RenderActionCenter(snap,az,fk,overrides,true);
}
function refreshDataHealth(){return Promise.resolve();}
