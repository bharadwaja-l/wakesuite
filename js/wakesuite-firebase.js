...
window.updateSuppressionCase=async function(id,patch={}){
  ...
  if(canDocs){if("poaStatus" in patch){allowed.poaStatus=patch.poaStatus;if(patch.poaStatus!==before.poaStatus)allowed.poaSubmittedOn=patch.poaSubmittedOn||nowText;}if("poaLink" in patch)allowed.poaLink=String(patch.poaLink||"").trim();if("qcStatus" in patch){allowed.qcStatus=patch.qcStatus;if(patch.qcStatus!==before.qcStatus)allowed.qcSubmittedOn=patch.qcSubmittedOn||nowText;}}
