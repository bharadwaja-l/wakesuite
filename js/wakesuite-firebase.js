...
window.updateSuppressionCase=async function(id,patch={}){
  const beforeCase=await getDoc(doc(db,"suppression_cases",id));const before=beforeCase.exists()?beforeCase.data():{};
  const enriched={...patch};const nowText=new Date().toISOString();
  if("caseId" in patch&&patch.caseId&&!before.caseId)enriched.caseIdRaisedOn=patch.caseIdRaisedOn||nowText;
  if("pocEscalationStatus" in patch&&patch.pocEscalationStatus&&patch.pocEscalationStatus!==before.pocEscalationStatus)enriched.pocEscalatedOn=patch.pocEscalatedOn||nowText;
  if("poaStatus" in patch&&patch.poaStatus&&patch.poaStatus!==before.poaStatus)enriched.poaSubmittedOn=patch.poaSubmittedOn||nowText;
  if("qcStatus" in patch&&patch.qcStatus&&patch.qcStatus!==before.qcStatus)enriched.qcSubmittedOn=patch.qcSubmittedOn||nowText;
  if("status" in patch&&patch.status==="Closed"&&before.status!=="Closed")enriched.closedOn=patch.closedOn||nowText;
  // Call the existing permission gate for the normal fields first.
  await ws93BaseUpdateSuppressionCase(id,enriched);
  const lifecycle={};["caseIdRaisedOn","pocEscalatedOn","poaSubmittedOn","qcSubmittedOn","closedOn","reactivatedOn","firstSeen","lastSeen","resolvedOn"].forEach(k=>{if(k in enriched)lifecycle[k]=enriched[k];});
  if(Object.keys(lifecycle).length)await setDoc(doc(db,"suppression_cases",id),{...lifecycle,updatedBy:auth.currentUser?.email||"",updatedAt:serverTimestamp()},{merge:true});
};
window.loadDataAdminAudit=async function(){
  ws93RequireSuperAdmin();const snap=await getDocsFromServer(collection(db,"system_audit_log")),rows=[];snap.forEach(d=>{const x={id:d.id,...d.data()};if(["processed_data_clear","source_version_delete","source_file_delete"].includes(x.action))rows.push(x);});return rows.sort((a,b)=>String(b.deletedAtText||b.createdAtText||"").localeCompare(String(a.deletedAtText||a.createdAtText||""))).slice(0,500);
};
window.wakeSuiteFirebase.version="9.3.0";
