...
## V9.2 module-local change rule
When a change concerns only one source/template/workflow, edit that module/config first and avoid modifying unrelated modules. Update `CHANGELOG.md` plus the applicable Business Rules / Architecture / Firestore document in the same release. AI / Ask WakeSuite is outside V9.2 scope.

## V9.3 consolidated-release rule
V9.3 was reviewed as a locked change set before implementation. Future multi-module releases should follow the same workflow when requested: review issue → lock requirement → maintain a master change list → implement once → run package validation → release one consolidated ZIP.

For wide-table additions, verify column selector registration and visible/full export behavior. For native Dashboard routing, verify destination + carried context + Back restoration. For source deletion, verify source-version removal and downstream invalidation are scoped to the selected source version/date only.
