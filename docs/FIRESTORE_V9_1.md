...
## V9.3.3 Firestore impact
No new Firestore collections/rules are introduced. Order Report raw versions continue using the existing local raw-file/version store; existing Firestore authorization and snapshot persistence remain authoritative for existing data.


## V9.3.4 Daily Communications field

`/suppression_cases/{caseId}` now permits the optional `poaLink` field for users with `managePoaQc`. This field is used only to populate the POA Link column in the Amazon ASIN Suppression POC attachment. Existing suppression workflow permissions remain unchanged.
