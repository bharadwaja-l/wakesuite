...
## V9.3.4 — Shared Analytics Filter Bar
- Added reusable analytics filter bar for Dashboard, analytical reports, Insights, History, and Business Insights.
- Standardized shared filter hierarchy: Period → Marketplace (where applicable) → Category → Identifier Search → Apply → Reset.
- Added active filter chips and responsive custom/selected-date controls.
- Added Last 14 Days support to Dashboard period compatibility without changing existing processing architecture.
- Preserved analysis-specific filters outside the shared bar.
